...
void setup()
{
  ...
  ServoFinger1_1.attach(3);  // The servomotors are asigned to the pins of the Arduino UNO board.
  delay(300);              // A delay of 300ms is set for a secure connection (this can be optional)
  ServoFinger2_1.attach(2);   
  delay(300);
  ServoFinger3_1.attach(4);
  delay(300);
  ServoFinger1_2.attach(5);
  delay(300);
  ServoFinger2_2.attach(6);
  delay(300);
  ServoFinger3_2.attach(7);
  delay(300);
    ...
    if(startPackage == '<'){   // Verifying that the first value is "<"
      ...
      if(AngFinger1_1 != -1)
        if(AngFinger1_1 != 255)  // Sometimes the incoming value goes to 255, I couldn't discover yet the reason, so I simply excluded it when it happens. You can remove this line for every finger if you don't have this kind of problem
          if(AngFinger1_1 >= AngFinger1_1Last+6  || AngFinger1_1 <= AngFinger1_1Last-6)
            AngFinger1_1Last = AngFinger1_1;
            ServoFinger1_1.write(AngFinger1_1);  // The servomotors rotates of the assigned degrees
